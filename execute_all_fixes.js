...
const verification = db.prepare(`
  SELECT 
    ...
    SUM(CASE WHEN Description LIKE '%```javascript%' THEN 1 ELSE 0 END) as questionsWithCode,
    (SELECT COUNT(*) FROM ChallengeSnippets cs JOIN Challenges c ON cs.ChallengeId = c.Id WHERE c.TestId = 2 AND cs.Code LIKE '%...%') as incompleteSnippets,
    (SELECT COUNT(*) FROM ChallengeSnippets cs JOIN Challenges c ON cs.ChallengeId = c.Id WHERE c.TestId = 2 AND LENGTH(cs.Code) < 50) as shortSnippets
  FROM Challenges 
  WHERE TestId = 2
`).get();

console.log('📊 Final Results:');
console.log(`Total Questions: ${verification.totalQuestions}`);
console.log(`Questions with Code: ${verification.questionsWithCode}`);
console.log(`Incomplete Snippets: ${verification.incompleteSnippets}`);
console.log(`Short Snippets: ${verification.shortSnippets}`);

if (verification.incompleteSnippets === 0 && verification.shortSnippets === 0) {
